...
export default function TeacherSoftwareCalculator() {
  ...
  const calculateSemStats = (subjects: Record<string, SubjRow[]>, semKey: string) => {
    ...
    for (const r of rows) {
      ...
      num += p * weight; den += weight; cnt += 1; // 과목 수도 가중치 적용 (수학/과학/정보는 2배로 카운트)
    }
  ...
  const calculateScore = (student: StudentData) => {
    ...
    if (atype === "검정고시") {
      if (gedSubjects.length === 0) return { courseScore: 0, attScore: 0, volScore: 0, bonusLeadership: 0, bonusAwards: 0, totalScore: 0 };
  ...
  const downloadResults = async () => {
    ...
    // 헤더 설정
    const headers = [
      "수험번호", "이름", "전형", "지원유형", "교과성적", "출결점수", "봉사점수", "리더십점수", "수상점수", "총점", "유효성"
    ];
    ...
    a.href = url;
    a.download = "소프트웨어마이스터고_계산결과.xlsx";
    a.click();
...
A`;
    ...
    const sampleData = [
      ...
      ["00000-00002", "김철수", "특별전형", "졸업생", multilineSubjectDataWithFree, multilineGradeDataWithFree, "15", "12", "10", "1", "0", "1", "1", "2", "0", "4", "2"],
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "소프트웨어마이스터고_샘플데이터.xlsx";
    a.click();
    window.URL.revokeObjectURL(url);
  };
